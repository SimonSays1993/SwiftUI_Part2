...
//
//  Created by Simon Mcneil on 2022-01-17.
//
...
struct CourseList: View {
    ...
    var body: some View {
        ZStack {
            ...
            // This looks good on simulator but making swiftUI preview animation add glitchy.
...
struct CourseView: View {
    ...
    var body: some View {
        ZStack(alignment: .top) {
            ...
            .padding(show ? 30 : 20)
            .padding(.top, show ? 30 : 0) //we add more top padding as dealing with safeArea notches when in full screen
            .frame(maxWidth: show ? .infinity : screen.width - 60, maxHeight: show ? 460 : 280)
            .background(Color(course.color))
            .clipShape(RoundedRectangle(cornerRadius: 30, style: .continuous))
            .shadow(color: Color(course.color).opacity(0.3),
                    radius: 20, x: 0, y: 20)
            .onTapGesture {
                self.show.toggle()
                self.active.toggle()
        ...
        .animation(.spring(response: 0.5, dampingFraction: 0.8, blendDuration: 0), value: show)
        .edgesIgnoringSafeArea(.all)
    }
}
